...
> 聊天分附近 / 世界两个频道，「附近只有同城镇的人看到」「保留最近几条历史」
> 都是用户 2026-08-26 明确定的 —— **世界频道现在只做了表结构，Reducer 还没写**。
...
| 城镇 NPC（配置表 `TownNpc`，站在城镇的固定世界坐标上） | — | ✅ 通路已通，**表还空着等配** | 同上 |
...
2. **聊天的世界频道** —— 附近消息 2026-08-26 已经做完并实测通过。世界频道的表结构、
   滚动裁剪、冷却全是共用的，缺的就是服务端一个 `SendWorldMessage`（域填
   `ChatWorldScopeTownId` = 0）+ 客户端多订一句 `WHERE town_id = 0`，
   发送入口在 `PopMessageUI`。用户说测通附近之后再做这一步。
3. **`MainCommonUI` 的功能按钮**：移动、右上角信息、聊天框已经通了，其余按钮还没有。
   **要动手前先问**要哪些。（右下角的 `OpenMessageUIButton` 故意还没接 ——
   它要开的是 `PopMessageUI`。）
4. **双客户端实测同城镇玩家**：渲染路径验过了（注入合成玩家 + 按 town_id 的订阅确实在收数据），
   但**两个真客户端同时在线没试过** —— 要出个包和编辑器对着跑一次。
5. **补配置的占位值**：`CharacterJob.Subtitle` 还空着，觉醒等级（15 / 30）是占位数字。
...
6. **再加城镇**：新城镇要做三个背景预制体（**世界空间 SpriteRenderer**，Sorting Layer
...
7. **城镇之间怎么走还没做**：服务端只有 `PlaceCharacter`（进游戏时落位），
   没有「玩家自己换城镇」的 Reducer —— 要先定城镇怎么解锁 / 能不能随便去，**要动手前先问**。
8. **升星**：现在只有觉醒（1→3→6 星），4 / 5 星没有来源 —— 要靠养成系统（材料 / 碎片），
   那套还没定，**要动手前先问**。
9. **爆发宝石**：配置已就绪，但「装备宝石切形态」是战斗内行为，
   装备 / 背包 / 战斗表一张都还没有，**要动手前先问**。
10. 地图 / 战斗 / 背包这些玩法态表 —— 还没设计，**要动手前先问**。

### NPC 表怎么填（2026-08-26 新加，现在是空表）

`ExcelTool/LubanTools/DataTables/Datas/TownNpc.xlsx`，一行一个 NPC，**纯客户端**
...
聊天消息表里留着 2026-08-26 的几条测试消息（兰德索尔，几个角色互相说话）。查 / 清：

```bash
spacetime sql rediv "SELECT message_id, town_id, sender_name, content FROM chat_message"
```
...
⚠️ `spacetime call rediv send_nearby_message '"你好"'` **一定会失败**（回「请先进入城镇再发言」）
—— 每条 CLI 命令都是一条新连接、没有选角行。要测发送只能用活着的客户端。
...
- 聊天的：**世界频道**（表结构已就绪，Reducer 和客户端入口没写，见「下一步」第 2 条）、
  禁言 / 拉黑 / 举报、私聊与队伍频道、头顶气泡、表情与 @某人
...
- 相机适配 —— **不是「有意没做」，是排在队首的下一件事**，见上面「下一步」第 1 条
- `MainCommonUI` 的功能按钮 —— 移动和信息栏已通，按钮待定
