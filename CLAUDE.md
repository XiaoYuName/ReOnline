...
> 服务端现有**账号系统**、**角色系统**、**城镇与世界时间**、**聊天**
> （`ReDiv_Server/spacetimedb/` 下的 `Auth/`、`Character/`、`Town/`、`Chat/`）。
> 前两个是通用基础设施；城镇和时间系统是**第一批玩法表**，形态和边界都是用户
> 2026-08-25 明确定的（见第 5 节），不是我们推的。
> 聊天分附近 / 世界两个频道，「附近只有同城镇的人看到」「世界所有人在任何地方都能看到」
> 「保留最近几条历史」「说话时头上冒气泡、5 秒收、上限 20 字」
> 全是用户 2026-08-26 明确定的，两个频道都已经做完。
...
- **表回调里的 `ctx.Event`：别人改的行拿到的是 `Transaction`，不是 `Reducer`。**
  `Reducer` 变体**只有自己调的那次**才有（2.x 没有全局 Reducer 回调）。
  所以「这条是不是刚发生的」要写成**排除 `SubscribeApplied`**，
  不能写成「是不是 `Reducer`」—— 后者在单客户端下完全正常，一联机就只剩自己有反应。
  说话气泡踩过这个，见 [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节「说话气泡」
...
- **本工程 Canvas 是 `ScreenSpaceCamera`，不是 Overlay**（2026-08-25 订正）。
  ...
  `capture_game_view` 默认渲 `MainCamera`，**UI 在 `UICamera` 上拍不到** ——
  要整屏（世界 + UI）得加 `--source screen`。细节见客户端文档第 5 节坑 10
- **城镇背景是世界空间的 `SpriteRenderer`，不是 UI**（2026-08-25 改的）。背景、角色、
  出生点、以后的边界全在世界坐标里，**美术钉死在世界里、适配交给相机** ——
...
- **`[AutoInc]` 的自增 id 不能拿来排序**。官方规则明确写着它不保证连续也不保证单调
  ...
  聊天消息就是这么排的，而且**服务端裁剪和客户端显示必须用同一把尺子** ——
  口径不一致会出现「客户端认为还该显示的那条被服务端当成最旧删了」
...
- **加一张新配置表要按顺序跑四步**：① 导出（ConfigTools 第 1 步）→ ② **自动打包 Addressable
  （打标）** → ③ 生成 AssetKeys → ④ 生成 LubanManager。少一步就编译不过，而且报错指向的地方
  很误导（`AssetKeys` 是从 **Addressables 条目**生成的，不是从文件系统 —— 导出完没打标就生成
  AssetKeys，新表的 `TbxxxPath` 就是没有）。2026-08-26 和 08-27 各踩了一次，
  细节见 [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 3 节
- **触发器和副本都是「纯客户端表现 + 服务端权威」**：`TownTrigger` / `DungeonArea` / `Dungeon`
  三张表整表 `group=c`（改完**不用 publish**），但**能不能去那个城镇是服务端 `ChangeTown` 说的**
  —— 改过的客户端可以对任意城镇调那个 Reducer。别把权限判断写在客户端的表里
- **国服还原出来的 VariantCard 材质拿去做 UI 背景，`renderQueue` 必须从 2000 改成 3000**。
  原包里是 2000，而 UGUI 默认 3000 ⇒ queue 小的先画 ⇒ **那张背景会被所有 UI 盖住**。
  症状极具误导性：「界面明明开着，别的界面元素还浮在上面」，看着像层级 / 兄弟序错了。
  2026-08-27 副本区域背景踩过，查错了两轮
...
| GM 锁时段（私有 `WorldTimeControl` + `refresh_world_time` 立即重算） | ✅ | — | 同上 |
| 城镇背景（**世界空间 SpriteRenderer**，两层：共用外层 + 按时段的背景） | — | ✅ 已通，兰德索尔三张背景已配（三张的 Sorting Layer 2026-08-26 统一成了 `Ground` / order -1，之前中午和晚上那两张是默认的 `Default:0`） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
| 城镇出生点（**每张背景**自己的 `StartPoints`，进城镇就落在那儿） | — | ✅ 已通 | 同上 |
| 可行走边界（**每张背景**自己的 `GroundCollider`，撞墙 + 沿墙滑） | — | ✅ 已通 | 同上 |
| 城镇 NPC（配置表 `TownNpc`，站在城镇的固定世界坐标上） | — | ✅ 已通，兰德索尔配了凯露 / 优衣两个 | 同上 |
| **传送**（**成对的传送阵**：走进 A → 服务端 `ChangeTown` → 出现在对端 B 的出口点旁） | ✅ | ✅ 2026-08-27 已通，来回都验过 | 同上 |
| **城镇触发器**（配置表 `TownTrigger`，矩形判定区。1=传送 2=开副本界面） | — | ✅ 纯客户端，摆位窗口已做（连对端 + 拖出口点） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
| 副本入口（走进触发器 → 打开 `PopDungeonUI`，`TargetId` = 副本区域 id） | — | ✅ | 同上 |
| **副本界面**（区域背景 + 副本列表 + 1~6 星难度切换，像 DNF） | — | ✅ 2026-08-27，**纯客户端**（配置 + 界面 + 选星级）。格子**按配置的 UI 坐标摆，不是平铺**；区域背景挂在界面自己层级里（不走 `UIBackground` 层） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
| 星级解锁（打过 N 星才能选 N+1） | — | ⚠️ 逻辑通了，但**通关进度是本地 PlayerPrefs 占位** —— 没有战斗就没人写它 | 同上 |
| 进入游戏（选人 → `SelectCharacter` → 城镇主界面 `MainCommonUI`） | ✅ | ✅ | 同上 |
| 城镇角色 + 摇杆移动（客户端上报坐标，服务端转发） | ✅ | ✅ 两层结构：外层控制器 + 按形态的 Spine | 同上 |
| 头顶名字（BoneFollower 跟骨骼 + 子节点做 Y 偏移） | — | ✅ | 同上 |
| 角色互相遮挡按 Y 排（站得靠下的盖住靠上的） | — | ✅ 2026-08-27 靠**全局** `Transparency Sort Mode = Custom Axis (0,1,0)`，零代码 | 同上 |
| 同城镇其他玩家（按 `town_id` 订阅，插值跟随） | ✅ | ✅ 渲染已通，**没做过双客户端实测** | 同上 |
| **聊天 —— 附近消息**（同城镇可见，保留最近 50 条 / 1 秒冷却） | ✅ | ✅ `MainCommonUI` 左下角聊天框已通 | [ReDiv_Server/README.md](ReDiv_Server/README.md)「聊天系统」 |
| **聊天 —— 世界消息**（全服可见，和附近**共用**冷却和保留窗口） | ✅ | ✅ `PopMessageUI` 附近 / 世界两个页签已通 | 同上 |
| **说话气泡**（角色头上，尺寸跟着文字变，5 秒自动收；上限 20 字） | — | ✅ 已通，**自己和别人的都验过了**（**纯客户端**，服务端不知道气泡这回事） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
| 右上角信息（等级 / 经验 / 体力 + 金币 / 钻石） | ✅ | ✅ | 同上 |
| 本地 Web GM（统计 / 日志 / 账号栏位 / 角色数值 / 世界时段锁定） | ✅ owner API + 私有控制表 | ✅ 独立网页 | 同上「本地 Web GM 工具」 |
| 配置表通路（Excel → Luban → 编进 wasm / 进 Addressables） | ✅ | ✅ | 同上「配置表」 |
| 角色美术资源（头像 / 略缩图 / 名字图 / 立绘 / 预览图 / UI Spine / **城镇 Spine** / 战斗 Spine） | — | ✅ 两个角色都配好了 | 同上「配置表」 |
| 城镇背景美术（兰德索尔早/中/晚三张） | — | ✅ 已配 | 同上 |
...
`MainCommonUI`（城镇主界面：背景 / 出生点 / 可行走边界 / 摇杆移动 / 右上角信息 /
同城镇其他玩家 / NPC / **左下角聊天框 + 说话气泡** / **踩触发器传送与开副本**，
其余功能按钮待做）、
`PopMessageUI`（聊天弹窗：**附近 / 世界两个页签**，世界消息从这里发）、
`PopDungeonUI`（副本界面：区域背景 / 副本列表 / 1~6 星难度切换已通，**进副本没做**）。细节见
[ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节。

### 下一步大概率是这些

1. **相机适配（固定视野盒 + 黑边）** —— 背景搬到世界空间之后**现在完全没有适配**：
   相机 `orthoSize` 恒定 5，可视高 10 个世界单位、宽 = 10×宽高比，窗口比例一变
   看到的世界范围就跟着变。方案 2026-08-25 已经定了，实现还没做：
   算出屏幕内最大的**居中 16:9 像素区**，同时赋给 `MainCamera` 和 `UICamera` 的
   `Camera.rect` ⇒ 相机 aspect 恒为 16:9、`orthoSize` 保持 5，一行缩放公式都不用，
   视口外的像素相机根本不渲染。**固定的是「看到多少世界」（17.7778 × 10 单位），
   不是「1 单位等于多少像素」**。⚠️ **不要做 cover / 按宽高比缩放美术** ——
   那会让不同宽高比的玩家看到的范围和距离都不一样，联机里是硬伤。
   还没定的只有一件：HUD（右上角信息栏 / 摇杆）要不要一起压进 16:9 盒子里。
2. **`MainCommonUI` 的其余功能按钮**：移动、右上角信息、聊天框、说话气泡、
   右下角「世界聊天」、踩触发器传送 / 开副本都已经通了，剩下的按钮还没定。
   **要动手前先问**要哪些。（`PopMessageUI` 里那个 `SettingsButton` 也还没接。）
3. **双客户端实测**：渲染路径验过了（注入合成玩家 + 按 town_id 的订阅确实在收数据），
   但**两个真客户端同时在线的场景仍然覆盖不全** —— 要出个包和编辑器对着跑一次。
   ⚠️ **这一条不是走过场**：说话气泡的判据写错过一次，症状是「联机下只有自己冒泡」，
   而**单客户端下三种测法全是绿的**（自己发自己看永远是 `Event.Reducer`）。
   2026-08-27 是用户联机时发现的。凡是「别人做了什么我要有反应」的功能，
   都要么真开两个客户端，要么用**另一条连接**（临时探针 Reducer + `spacetime call`）
   模拟 —— 后者便宜很多，气泡最后就是这么验的。
   聊天还欠的具体一条：**「两个客户端分别在不同城镇，世界消息互通、附近消息互不可见」
   还没真验过**。⚠️ 这条 2026-08-27 起**可以验了** —— 传送做完之后有了第二个城镇
   （`TownId=2` 测试镇，虽然背景是复用的），两个客户端各自传送过去就能测跨城镇的
   聊天隔离性。以前拦路的「只有一个城镇」这个理由已经不成立了。
...
5. **再加城镇**（现在最缺的就是这个 —— 传送已经能用，但只有一个真城镇）：
   新城镇要做三个背景预制体（**世界空间 SpriteRenderer**，Sorting Layer
   `Ground`，比例烘在 localScale 里），每张上面还要有 `StartPoints`（出生点）和
   `GroundCollider`（可行走边界，**物理层必须是 `Ground`**）+ `TownGroundController` 组件，
   路径填进 `Town.xlsx` 的三列。**别手打路径**的录入窗口还没做，城镇少暂时手填。
   ⚠️ `Town.xlsx` 里现在有一行 **`TownId=2` 测试镇**，三列背景**复用兰德索尔那三张** ——
   那是 2026-08-27 为了验传送加的临时行（只有一个城镇的话传送验不了）。
   第二个城镇的美术出来之后**直接替换那三列**，或者删掉这一行（连带
   `TownTrigger.xlsx` 里 `TownId=2` 的那一行和兰德索尔那个指向它的传送点）。
   ⚠️ 出生点和边界是**一张图一套**（早/中/晚三张各自一份），三份要保持一致 ——
   改了一张记得另外两张一起改，不然玩家跨时段会发现「白天能走的地方晚上走不了」。
6. **城镇解锁规则还没定**：传送本身通了（`ChangeTown`），但现在是**所有城镇都能去**
   （用户 2026-08-27 定的：只有一个真城镇，做复杂规则也验不出来）。
   要按等级 / 进度收紧的话**只改 `ChangeTown` 里那一处 TODO**，别散到客户端 ——
   客户端的触发器是纯表现，改过的客户端能对任意城镇调那个 Reducer。**要动手前先问**规则。
7. **升星**：现在只有觉醒（1→3→6 星），4 / 5 星没有来源 —— 要靠养成系统（材料 / 碎片），
   那套还没定，**要动手前先问**。
8. **爆发宝石**：配置已就绪，但「装备宝石切形态」是战斗内行为，
   装备 / 背包 / 战斗表一张都还没有，**要动手前先问**。
9. **副本里面装什么** —— 界面通了（选副本 + 选 1~6 星），但**副本本身一张玩法表都没有**：
   关卡内容、战斗、结算、体力消耗、掉落、需求等级、通关进度存哪。
   ⚠️ 这里有个**已知的半成品**：星级解锁按「打过 N 星才能选 N+1」算，而通关进度
   现在是**本地 PlayerPrefs 占位**（`DungeonProgress`）—— 接结算时要换成服务端表。
   **要动手前先问**。
10. **组队** —— 副本界面的选择已经收敛到 `DungeonSelection`（队长写 / 队员只读的口子留好了），
    但队伍系统本身（组队、队长、邀请、队伍表）一张表都没有。**要动手前先问**。
11. 地图 / 战斗 / 背包这些玩法态表 —— 还没设计，**要动手前先问**。

### NPC / 触发器 / 副本格子怎么摆

**坐标别手填** —— 三个摆位窗口，都在 `Tools > XFramework > 配置` 下，流程一模一样：
选目标 → 打开预览 → 在 Scene 视图里直接拖 → 「写入 Excel」（默认顺手重新导出配置）。

| 窗口 | 表 | 摆什么 | 坐标系 |
|---|---|---|---|
| `NPC 摆位` | `TownNpc.xlsx` | NPC 站的位置、朝向、Spine 预制体 | **世界** |
| `触发器摆位` | `TownTrigger.xlsx` | 传送点 / 副本入口的矩形判定区 + 传送的**出口点**（位置在场景里拖，宽高在窗口的表里填） | **世界** |
| `副本摆位` | `Dungeon.xlsx` | 副本界面里每个格子的位置 | **UI** |

⚠️ **最后那个是 UI 坐标**，所以它的预览挂在真实 Canvas 下
（`UISystem/UILayout/UIPanel`）而不是世界原点 —— UI 坐标要过 `CanvasScaler` 才有意义。
前两个是世界坐标，预览直接丢在原点。

三张表都是**纯客户端**（`group` 全是 `c`，改完**不用 `spacetime publish`**）。
列的含义和实现约束见 [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md)
第 5 节「城镇 NPC」/「城镇触发器」/「副本界面」和第 4 节那三个窗口 —— 别在这儿再抄一份。

⚠️ **服务端不认识触发器**。「能不能去那个城镇」是服务端 `ChangeTown` 自己校验的，
所以在 `TownTrigger.xlsx` 里改目标城镇**不等于**改了权限。
...
alice 的钱包被测试改成了 `coin=12345 / gem=678`（要清零跑
`spacetime sql rediv "UPDATE account_wallet SET coin=0, gem=0 WHERE account_id=1"`）。
体力是**每日重置**的，查当前值：
`spacetime sql rediv "SELECT character_id, level, stamina, stamina_day FROM character"`。

配置侧是稳定的：`JobId=1` 凯露 / `JobId=2` 优衣，都 `MaxStar=6`、`StartStar=1`，
每个角色四行形态（基础线 3 个 + 爆发线 1 个）—— 具体形态名和解锁条件见
[ReDiv_Server/README.md](ReDiv_Server/README.md)「现在配了哪些角色」，别在这儿再抄一份。
城镇有两个：`TownId=1` 兰德索尔（初始城镇）和 `TownId=2` **测试镇** ——
后者是 2026-08-27 为了验传送加的**临时行**，三列背景复用兰德索尔那三张
（见「下一步」第 5 条）。时段边界 5 / 11 / 18 点。
副本：`DungeonArea` 一个区域 `31001` 幽暗密林（背景 `bg_500170_Preview`），
`Dungeon` 四个小副本 `31005`~`31008`（`MaxStar` 分别是 3/6/6/1）。
格子的 UI 坐标现在是**均匀横排的占位值**（y=0，x = -576/-192/192/576）——
用 `副本摆位` 窗口对着背景摆成设计要的样子。
⚠️ **四个副本名（密林入口 / 迷雾小径 / 古树之下 / 密林深处）是我编的占位名**，
略缩图是按 id 对上的那四张 —— 名字和 `MaxStar` 都请策划改。
通关进度存在 **PlayerPrefs**（占位），调试用
`DungeonProgress.DebugSetCleared(副本id, 星级)` / `DebugClearAll()`。

触发器 `TownTrigger.xlsx` 三行，**传送是成对的**：
兰德索尔 `#1`「前往测试镇」(-8.85, -4.03) ⇄ 测试镇 `#3`「返回兰德索尔」(7.5, -3.3)
（`TargetId` 互指），外加兰德索尔 `#2` 副本入口 (8.45, -4.23)，
它的 `TargetId=31001` 指向副本区域「幽暗密林」。
出口偏移 `#1` 是 (+1.2, 0)、`#3` 是 (-1.2, 0)，都指向城里那一侧。
⚠️ `#3` 的位置和两个出口偏移**是我估的**，用 `触发器摆位` 窗口对着背景挪一下
（`#1`/`#2` 的位置是用户自己拖的）。
等级表 `TbLevelExp` 是 1~60 级，`ExpToNext` / `MaxStamina` 都是**占位数值**（待策划调）。
...
聊天消息表里留着 2026-08-26 的几条测试消息（附近和世界各几条）。查 / 清
（`channel` 1=附近 2=世界，`town_id` 0 就是世界域）：

```bash
spacetime sql rediv "SELECT message_id, channel, town_id, sender_name, content FROM chat_message"
```

```bash
spacetime sql rediv "DELETE FROM chat_message"
```

⚠️ `spacetime call rediv send_nearby_message '"你好"'`（`send_world_message` 同理）
**一定会失败**（回「请先进入城镇再发言」）—— 每条 CLI 命令都是一条新连接、没有选角行。
要测发送只能用活着的客户端。
...
> 只列**已经想过、故意不做**的。做完了的去上面那张进度表，
> 排队要做的去「下一步」—— 三处各归各位，别在这儿堆已完成项。

- 登录失败次数锁定（做不了，原因是事务回滚，见服务端 README「有意没做的事」）
  ⚠️ 注意**聊天的发言冷却是能做的**、而且已经做了（1 秒）—— 区别在于它只在
  **成功**那条路上留痕迹（消息本身就是记录），被拒的那次回滚什么都不用存
- 改密码 / 找回密码 / 删号 / 改角色名 / 软删恢复 / 扩栏位入口 / 敏感词过滤
- 聊天的：禁言 / 拉黑 / 举报、私聊与队伍 / 公会频道、表情与 @某人、
  世界频道的**独立冷却 / 发言等级门槛 / 喇叭道具**（现在世界和附近共用 1 秒冷却，
  全服喊话没有任何额外成本）
- 气泡靠屏幕右边时**翻到左侧显示** —— 现在只会往右长，贴右边的角色说长话会出画
- 玩法态表（战斗、地图、背包、装备）—— 玩法未定型，**不要自己建**。
  城镇和世界时间是例外，已经建了（用户 2026-08-25 指定的）；
  副本的**配置表**（区域 / 小副本）也建了，但那两张是**纯客户端**的，
  副本的玩法态（进副本、结算、掉落）一张都还没有
- **副本的服务端那一半** —— 现在服务端完全不认识副本：没有「进副本」Reducer、
  不扣体力、不存通关进度。星级解锁用的是本地 PlayerPrefs 占位（用户 2026-08-27 定的本轮范围）
- **副本界面的选中高亮** —— `DungeonSlot` 预制体里没有选中态的美术节点，
  所以点了格子只是记下 `DungeonSelection.SelectedDungeonId`，视觉上看不出来。
  组队时「队长选了哪个」要能看出来，那时得让美术加个高亮节点
- **`btn_dungeon_31001`~`31004` 那 4 张图还没用上** —— 看着像副本区域的按钮，
  等做「区域选择界面」时再说（现在是踩触发器直接进某个区域）
- **城镇解锁规则** —— 传送（`ChangeTown`）通了，但现在**所有城镇都能去**。
  收紧的口子留在那个 Reducer 里的一处 TODO（用户 2026-08-27 定的）
- **传送的冷却 / 读条 / 转场动画** —— 现在踩到就瞬间换城镇，没有任何缓冲。
  服务端也没有最小间隔（要加就比对 `CharacterLocation.EnteredAt`）
- **触发器的地面标记美术** —— `TownTrigger.IconPrefab` 那一列已经通了（配了就实例化），
  但**现在三个触发器一个都没配图**，所以传送点和副本入口在游戏里是看不见的
  （只有 Scene 视图里有 Gizmo 框：蓝=传送、橙=副本、绿圈=出口点）
- **触发器只有矩形、只按脚下一点判定** —— 圆形 / 多边形、以及「需要按一下才生效」那种
  交互式触发器都没做（用户 2026-08-27 明确选的「传送自动、副本弹界面」）
- **传送阵不要求互指** —— A 连 B 不代表 B 连 A（单向传送门是合法设计），
  校验只看「对端存在、是传送点、在别的城镇」。要做「必须成对」的检查得另外加
- 城镇背景资源的录入窗口 —— 城镇少，那三列路径暂时手填（NPC 有摆位窗口，背景没有）
- 移动的防作弊校验 —— 服务端只转发坐标，改客户端能瞬移（用户 2026-08-25 明确选的，
  城镇里瞬移没收益）。要管就在 `UpdateTransform` 里比对时间差和距离
- 经验 / 体力 / 金币的**获取途径** —— 表和显示都有了，但没有任何加它们的接口
  （升级、副本扣体力、掉落）。那要等副本和养成，**动手前先问**
