...
| **说话气泡**（角色头上，尺寸跟着文字变，5 秒自动收；上限 20 字） | — | ✅ 已通（**纯客户端**，服务端不知道气泡这回事） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
...
3. **双客户端实测**：渲染路径验过了（注入合成玩家 + 按 town_id 的订阅确实在收数据），
   ...
   聊天这边同样欠这一步：附近消息的隔离性是靠探针往 town 99 插消息验的，
   **「两个客户端分别在不同城镇，世界消息互通、附近消息互不可见」还没真验过**
   （现在只有一个城镇，第二个城镇也还没做）。
