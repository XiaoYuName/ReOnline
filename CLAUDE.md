...
`PopMessageUI`（聊天弹窗：**附近 / 世界两个页签**，世界消息从这里发）。细节见
...
2. **`MainCommonUI` 的其余功能按钮**：移动、右上角信息、聊天框、说话气泡、
   右下角「世界聊天」都已经通了，剩下的按钮还没定。**要动手前先问**要哪些。
   （`PopMessageUI` 里那个 `SettingsButton` 也还没接。）
...
6. **城镇之间怎么走还没做**：服务端只有 `PlaceCharacter`（进游戏时落位），
...
表是 `ExcelTool/LubanTools/DataTables/Datas/TownNpc.xlsx`，**纯客户端**
（`group` 全是 `c`，服务端没有 NPC 概念，改完**不用 publish**），
改完跑 ConfigTools 第 1 步（导出）就生效。
列的含义和实现约束见
[ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节「城镇 NPC」—— 别在这儿再抄一份。
