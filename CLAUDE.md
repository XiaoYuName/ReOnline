...
| 副本入口（走进触发器 → 打开 `PopDungeonUI`） | — | ✅ **只做到「打开界面」**，界面内容和副本本身都还没设计 | 同上 |
...
`MainCommonUI`（城镇主界面：背景 / 出生点 / 摇杆移动 / 右上角信息 / 同城镇其他玩家 /
**左下角聊天框 + 说话气泡**，其余功能按钮待做）、
`PopMessageUI`（聊天弹窗：**附近 / 世界两个页签**，世界消息从这里发）、
`PopDungeonUI`（副本界面：**壳子已搭好、逻辑没接**，走进副本触发器就打开它）。细节见
...
- **触发器的地面标记美术** —— `TownTrigger.IconPrefab` 那一列已经通了（配了就实例化），
  但**现在两个触发器都没配图**，所以传送点在游戏里是看不见的（Scene 视图里有 Gizmo 框）
