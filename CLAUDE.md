...
| **副本界面**（区域背景 + 副本列表 + 1~6 星难度切换，像 DNF） | — | ✅ 2026-08-27，**纯客户端**（配置 + 界面 + 选星级）。区域背景挂在界面自己层级里（不走 `UIBackground` 层） | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
...
**坐标别手填** —— 两个摆位窗口，都在 `Tools > XFramework > 配置` 下，用法一模一样：
选城镇 → 打开预览（把那张背景和东西一起生成到场景里）→ 在 Scene 视图里直接拖 → 「写入 Excel」。

| 窗口 | 表 | 摆什么 |
|---|---|---|
| `NPC 摆位` | `TownNpc.xlsx` | NPC 站的世界坐标、朝向、Spine 预制体 |
| `触发器摆位` | `TownTrigger.xlsx` | 传送点 / 副本入口的矩形判定区（**位置在场景里拖，宽高在窗口的表里填数字**） |

两张表都是**纯客户端**（`group` 全是 `c`，改完**不用 `spacetime publish`**），
改完跑 ConfigTools 第 1 步（导出）就生效 —— 两个窗口默认写完就顺手重新导出。
列的含义和实现约束见 [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节
「城镇 NPC」/「城镇触发器」—— 别在这儿再抄一份。
