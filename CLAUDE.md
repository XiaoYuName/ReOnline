...
| 城镇出生点（背景外层控制器上的 `StartPoint`，进城镇就落在那儿） | — | ✅ 已通 | 同上 |
...
5. **再加城镇**：新城镇要做三个背景预制体（**世界空间 SpriteRenderer**，Sorting Layer
   `Ground`，比例烘在 localScale 里），路径填进 `Town.xlsx` 的三列。
   **别手打路径**的录入窗口还没做，城镇少暂时手填。
   ⚠️ **出生点现在在共用外层预制体的 `StartPoint` 上，也就是所有城镇同一个落点** ——
   第二个城镇进来时要先定怎么按城镇分（外层做预制体变体 + 配置加一列，
   或者让内层背景带一个可选的 `StartPoint` 覆盖），**动手前先问**。
...
- **可行走边界** —— 地图不是所有位置都能走，但边界**先不做**（用户 2026-08-25 定的：
  他要先导一个插件进来再处理）。所以现在摇杆能一直走出画面。
  做的时候挂**背景外层控制器**那一层（出生点已经在那儿了），别另开资产或配置列
