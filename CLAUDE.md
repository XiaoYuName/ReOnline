...
| 城镇背景（**世界空间 SpriteRenderer**，两层：共用外层 + 按时段的背景） | — | ✅ 已通，兰德索尔三张背景已配 | [ReDiv_Online/CLAUDE.md](ReDiv_Online/CLAUDE.md) 第 5 节 |
...
`MainCommonUI`（城镇主界面：背景 / 出生点 / 摇杆移动 / 右上角信息 / 同城镇其他玩家，功能按钮待做）。细节见
...
2. **`MainCommonUI` 的功能按钮**：移动和右上角信息已经通了，按钮还没有。
   **要动手前先问**要哪些。
