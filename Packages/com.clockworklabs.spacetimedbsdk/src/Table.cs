...
namespace SpacetimeDB
{
        ...
        where Row : class, IStructuralReadWrite, new()
    {
        ...
        // ReDiv 本地改动（见 ../UPSTREAM.md）：删掉了原本重置 `_serializer` 的
        // [RuntimeInitializeOnLoadMethod] 方法。
        //
        // Unity 无法在泛型类的方法上兑现这个特性 —— 本类是
        // RemoteTableHandleBase<EventContext, Row>，所以 Unity 每次域重载都会报
        ...
        //
        // 而它本来也不需要：`_serializer` 只是 Row 对应序列化器的懒加载缓存，
        // 那个序列化器无状态且不可变，跨域重载残留是无害的。
