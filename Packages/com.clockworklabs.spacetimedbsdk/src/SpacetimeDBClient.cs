...
namespace SpacetimeDB
{
        ...
        where Tables : RemoteTablesBase
    {
        ...
        // ReDiv 本地改动（见 ../UPSTREAM.md）：删掉了原本重置 `IsTesting` 的
        // [RuntimeInitializeOnLoadMethod] 方法。
        //
        // Unity 无法在泛型类的方法上兑现这个特性 —— 本类是
        // DbConnectionBase<DbConnection, Tables, Reducer>，所以 Unity 每次域重载都会报
        //   Method 'ResetStaticFields' is in a generic class,
        //   but [RuntimeInitializeOnLoad] methods cannot be in generic classes
        // 并且从不调用它。也就是说这个重置从来没生效过。
        //
        // 而它本来也不需要：`IsTesting` 只在 tests~/SnapshotTests.cs 里被赋过 true，
        // 那个目录带 `~` 后缀，Unity 根本不导入，所以在 Unity 工程里这个标志永远是
        // 字段初始化器给的 false，跨域重载残留也没有任何影响。
