...
⚠️ **副本是纯客户端的**（2026-08-27）：客户端有了「副本区域 → 小副本 → 选 1~6 星」的界面，
但**服务端一张副本表都没有** —— 没有进副本的 Reducer、不扣体力、不存通关进度。
那两张配置表（`DungeonArea` / `Dungeon`）整表 `group=c`，服务端根本看不到。
细节和接的时候要做什么见「待定」。
...
└── spacetimedb/            模块源码（spacetime.json 的 module-path 指向这里）
    ...
    ├── Chat/               聊天
    │   ├── ChatTables.cs         ChatMessage（公开，靠 TownId 分域）+ 频道 / 保留窗口 / 冷却常量
    │   ├── ChatReducers.cs       SendNearbyMessage + 冷却检查 + 滚动裁剪
    │   └── ChatRules.cs          正文清洗与长度规则（黑名单，和角色名的白名单不是一套）
    ├── Town/               城镇 / 世界时间 / 玩家状态
...
**每次进城镇都站到出生点**，坐标取自客户端背景预制体上的 `StartPoints` 节点；
边界是同一张预制体上的 `EdgeCollider2D`，客户端自己挡自己（服务端**不校验**，
改过的客户端照样能走出去 —— 和坐标一样，城镇里没收益）。用户 2026-08-25 明确定的**不记住上次站的位置** ——
...
源码在 `spacetimedb/Chat/`。**两个频道都做完了**（附近 2026-08-26、世界同日）。

### 两个频道，一张表，靠 TownId 分域

| 频道 | `Channel` | `TownId` | 谁看得到 |
|---|---|---|---|
| 附近 | 1 | 发送者所在城镇 | 只有**同城镇**的人 |
| 世界 | 2 | `0`（`ChatWorldScopeTownId`） | 所有人，在任何地方 |

**为什么是「一张表 + TownId 分域」，而不是两张表、也不是 `WHERE channel = x AND town_id = y`**：
订阅 SQL 是静态字符串，条件越少越不容易写错。把「世界」编码成 0 号域之后，两个频道的
订阅退化成同一个形状 `WHERE town_id = N`，客户端只要换个数字。城镇 id 是正数
（`town_config_self_test` 守着这条），所以 0 永远不会和某个真城镇撞上。

`Channel` 列仍然留着 —— 客户端要按频道上色 / 加前缀，而这个信息**不能**从 TownId 反推。

### 为什么是持久表而不是事件表

玩家进城镇要能看到最近几条对话（用户 2026-08-26 定的）。事件表（`Event = true`，
推完就删）零存储零裁剪，但那样进城镇聊天框是空的、订阅切换那一瞬间发的消息会丢、
`spacetime sql` 也查不到（不好调试）。

代价是要**滚动裁剪**：每个域只保留最新 `ChatHistoryPerScope`（现在 50）条，
每次插入后把多出来的最旧那些删掉。50 这个数同时决定三件事，改的时候一起想：
玩家能回看多少条、每发一条要扫多少行、客户端订阅生效时一次性收到多少行。

### 排序：`(SentAt, MessageId)`，**不能只按 MessageId**

官方规则明确写着自增 id 不保证连续也不保证单调
（[CLAUDE.md](CLAUDE.md) 的 Critical Rules 第 4 条：
「Auto-increment IDs are not sequential. Gaps are normal, do not use for ordering」）。
所以 `SentAt` 是排序权威，`MessageId` 只当**平局裁判** —— 时间戳一模一样时
（同一事务插多条就会这样）得有个稳定顺序，否则客户端每次重排结果都可能不同。

⚠️ **服务端裁剪和客户端显示必须用同一把尺子。** 两边排序口径不一致的话，
会出现「客户端认为还该显示的那条被服务端当成最旧删了」。

### Reducer

| 客户端绑定 | CLI | 行为 |
|---|---|---|
| `SendNearbyMessage(content)` | `send_nearby_message` | 发一条附近消息（域 = 自己所在城镇） |
| `SendWorldMessage(content)` | `send_world_message` | 发一条世界消息（域 = 0） |

**两个 Reducer 都是薄壳子**：鉴权 / 清洗 / 冷却 / 裁剪全部共用 `InsertChatMessage`，
**区别只有一个 —— 写进哪个域**。以后加队伍 / 公会频道照这个形状再加一个壳子，别另起一套。

**「发到哪个域」和「发送者是谁」都从服务端的选角行读，不收客户端参数** ——
那一行是 `SelectCharacter` 写的，客户端伪造不了。让客户端传 townId 的话，
改过的客户端就能往任意城镇喊话；让它传名字的话就能冒充别人说话。

世界频道**也要求本连接已经选了角**：它虽然和城镇无关，但发言人得有名字有头像，
那些都在选角行上；顺带也挡住了「还在选人界面就往全服喊话」。

消息行里冗余了发送者的 `SenderName` / `SenderJobId` / `SenderFormId`，都是
**发送那一刻的快照**。为什么不让客户端去 join：`Character` 是私有表客户端订阅不到；
`CharacterSelection` 虽然有这些字段，但那个人一离开城镇 / 下线行就没了 ——
于是历史消息会变成「无名氏说了句话」。**世界频道更硬**：说话的人可能在**另一个城镇**，
本地根本没订阅到他的选角行，想 join 也 join 不到。
客户端拿 `(SenderJobId, SenderFormId)` 去配置表 `CharacterForm` 取 `IconKey` 当头像
（头像挂在**形态**上，觉醒之后会变，所以存形态不只存角色）。

⚠️ `SenderJobId` / `SenderFormId` 是 2026-08-26 **后追加的两列**，所以只能加在
struct 末尾并带 `[Default(0)]`（见「环境地雷」：插到中间会被判成列重排，publish 直接拒）。
追加时 publish 会警告「All clients will be disconnected」，那是加列的正常代价。

⚠️ 没有选角行时**抛异常而不是静默返回**（和 `UpdateTransform` 相反）：
坐标上报是「尽力而为」的状态同步，丢一个包无所谓；发消息是玩家的一次明确操作，
失败了必须让他看到「怎么没发出去」。

### 冷却：1 秒，**不另开表**

同一个角色两条消息之间至少隔 `ChatCooldownMicros`（1 秒）。判断方式是按发送者索引
`Filter` 一遍找他最近一条消息的 `SentAt` —— **消息本身就是那条记录**，不用为它建
「上次发言时间」表。扫的行数上限就是保留窗口（50），通常远少于这个数。

⚠️ **冷却是跨频道共享的**（按发送者算，不分频道）。不然「附近发一条、世界发一条」
就能把 1 秒的限流翻倍，等于没限。代价是刚在附近说完话马上发世界会被挡 —— 这是有意的。

⚠️ 这个限流**能**做，而登录失败次数锁定做不了 —— 区别在于写的时机：
这里只在**成功**那条路上留下痕迹，被拒的那次抛异常回滚、什么都不用存；
账号那边要存的恰恰是「失败」，而失败必然回滚（见「账号系统 → 有意没做的事」）。

裁剪把他的消息全删干净了 ⇒ 找不到任何一条 ⇒ 放行。这是**有意的**：
那说明这个域里已经过去 50 条消息，冷却早就该过了。

### 正文规则：黑名单，**和角色名的白名单不是一套**

角色名走**白名单**（只放汉字 / 字母 / 数字 / 下划线），因为它要全服唯一、视觉混淆
会造出「看着同名的两个角色」。聊天正文是一次性的、不做唯一性判定 ——
白名单会把标点、括号、数学符号全挡掉，那不是安全，那是没法聊天。

所以 `ChatRules` 走黑名单，只挡三类：**控制字符**（塞进 UI 是乱码、塞进日志能伪造换行）、
**零宽字符**（看不见，用来绕过以后的敏感词过滤和刷屏）、**双向文本控制符**
（能让后面的文字反向渲染、伪装成别人说的话）。标点、emoji、外文一律放行。

另外做两件清洗（不是拒绝）：制表符 / 换行**折成空格**（玩家从别处粘贴很常见，
而聊天框那一行是单行显示的），连续空白压成一个、首尾去掉（「一堆空格顶屏」
是最省事的刷屏手段；U+00A0 和 U+3000 一起压，它们肉眼和普通空格一样）。

长度按**显示宽度**算（非 ASCII 算 2、ASCII 算 1），上限 60 ⇒ 中文 30 字 / 英文 60 字。
**故意不做精确的东亚宽度判定** —— 那要一张 Unicode 宽度表，而模块跑在
InvariantGlobalization 下拿不到。上限本来就是排版保护，宁可紧一点。

⚠️ **客户端的长度上限比这里严，两边故意不一样，不是漂移**（用户 2026-08-26 定的
「20 个字，这个客户端限制就可以了」）：客户端 `ChatValidation.MaxChars` = 20 个字
（按字符数）是**策划规则**，跟着说话气泡的排版走；这里的 60 是**防滥用天花板**。
客户端严于服务端 ⇒ 正常客户端放过去的东西服务端一定不会拒。反过来，改过的客户端
能发到 30 个汉字 —— 气泡有最大宽度会换行，不会顶炸排版，所以先这样。
真要卡死就把 `ChatRules.MaxDisplayWidth` 换成按字符数 20。

**没有敏感词过滤**，和角色名一致。

### 实测确认过的（2026-08-26，Play 模式 + CLI 双向验过）

| 验的东西 | 结果 |
|---|---|
| 客户端点「发送」→ 消息入库 → 推回来 → 渲染成一行 | ✅ 全链路通 |
| 别的城镇（town 99）插消息，本客户端（town 1）收不到 | ✅ 订阅过滤真的生效 |
| 一个域插到 55 条后再发一条 ⇒ 裁回 50，删的是最旧的 | ✅ |
| 同一事务插入的多条（`SentAt` 完全相同）顺序稳定 | ✅ `MessageId` 兜住了 |
| 离开城镇 ⇒ 聊天清空退订；换个角色重进 ⇒ 50 条历史回来 | ✅ |
| 服务端冷却（绕过客户端 IsBusy，第一条完成即刻发第二条） | ✅ 「说话太快了，稍等一下再发」 |
| 绕过 `ChatValidation` 直接调 Reducer 发超长 / 零宽 / 纯空白 | ✅ 三条全被服务端自己拒掉，文案一致 |
| **世界消息**：`send_world_message` 写进 `channel=2 / town_id=0` | ✅ 客户端订阅到并渲染出头像 |
...
⚠️ **`spacetime call rediv send_nearby_message '"你好"'`（世界那个同理）一定会失败**，
回「请先进入城镇再发言」—— 每条 CLI 命令都是一条新连接、没有选角行。
这不是 bug，正好是那条鉴权路径的验证。要测发送成功只能用活着的客户端。
...
- **敏感词过滤**（和角色名一致）
- **禁言 / 拉黑 / 举报** —— 要先定清楚谁有权禁言、禁多久、存在哪
- **私聊 / 队伍 / 公会频道** —— 域的编码方式已经留好了（TownId 那一列），
  但「谁能看到」的规则各不相同，玩法没定型之前不开这个口子
...
`../ReDiv_Online/ExcelTool/LubanTools/DataTables/Datas/` 下**九张**数据表
...
`PopDungeonUI`（副本界面：区域背景 + 副本列表 + 1~6 星难度切换，**纯客户端**，服务端不认识副本）。
...
- **副本的服务端那一半**：客户端 2026-08-27 做完了「区域 → 小副本 → 选 1~6 星」的界面
  （**纯客户端**，两张配置表整表 `group=c`，服务端完全不认识副本），
  但服务端这边一张表都没有：进副本的 Reducer、体力消耗、结算、掉落，
  以及**通关进度**（星级解锁要它 —— 客户端现在用 PlayerPrefs 占位，改过存档就能全解开）。
  接的时候：① 新开一张「角色 × 副本 × 最高通关星级」表，客户端那个 `DungeonProgress`
  换成它的 View（两个方法签名不用动，界面不用改）；② `Dungeon.MaxStar` 那一列要开给服务端
  （现在是 `c`），否则校验不了星级越界
- **队伍**：副本界面的选择已经收敛到客户端的 `DungeonSelection`（队长写 / 队员只读的口子
  留好了），但队伍系统本身（组队 / 队长 / 邀请 / 队伍表）一张表都没有
