...
**当前状态：账号系统 + 角色系统 + 城镇（含移动同步、玩家状态）与世界时间 + 聊天（附近频道）。**
注册 / 登录 / 会话，多角色的创建 / 删除 / 选择，形态与觉醒（基础 → 一觉 → 二觉，按星级现算），
「角色在哪个城镇」「现在是早/中/晚哪个时段」，以及**同城镇可见的附近消息**都能用了。
战斗、背包这些玩法表还没有 —— 定型后再往里加。

> ⚠️ 聊天的**世界频道还没做**（用户 2026-08-26 的顺序：先做附近，测通了再做世界）。
> 表结构、裁剪、冷却都已经是共用的，见「聊天系统」一节。
...
源码在 `spacetimedb/Chat/`。**现在只有附近频道**（用户 2026-08-26 的顺序：先做附近，
测通了再做世界）。
...
| 世界（**待做**） | 2 | `0`（`ChatWorldScopeTownId`） | 所有人，在任何地方 |
...
| 客户端绑定 | CLI | 行为 |
|---|---|---|
| `SendNearbyMessage(content)` | `send_nearby_message` | 发一条附近消息 |

**「发到哪个城镇」和「发送者叫什么」都从服务端的选角行读，不收客户端参数** ——
那一行是 `SelectCharacter` 写的，客户端伪造不了。让客户端传 townId 的话，
改过的客户端就能往任意城镇喊话。

发送者名字是**发送那一刻的快照**（冗余进消息行）。为什么不让客户端去 join：
`Character` 是私有表客户端订阅不到；`CharacterSelection` 虽然有名字，
但那个人一离开城镇 / 下线行就没了 —— 于是历史消息会变成「无名氏说了句话」。
...
⚠️ **`spacetime call rediv send_nearby_message '"你好"'` 一定会失败**，回
「请先进入城镇再发言」—— 每条 CLI 命令都是一条新连接、没有选角行。
...
- **头顶气泡**（说话时在角色头上冒一句）—— 城镇角色的外层控制器上还没有这个挂点
- 消息里的**表情 / 超链接 / @某人**
